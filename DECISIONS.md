...
### ADR-026: Stage 6F Real-Sim Sizing Push to Spec (Miller 60dB/40MHz/60deg PASS)
- **Date**: 2026-09-09
- **Status**: Accepted
- **Context**: 6E left an honest FAIL (gain 42.40dB, UGB 4.70MHz). The user authorized a genuine fix: reach the Stage 6 benchmark spec with real simulations. Two further defects surfaced on the way: (1) the sizing loop aborted the whole study on the first failing trial instead of recording it (violates the Stage 4 failures-are-data rule — invisible while the estimator never raised); (2) the cost ignored phase margin. Study 1 (wide 12-dim space incl. L, seed 42, 24 trials): best trial 1 at gain 77dB/UGB 30MHz — gain and UGB trade against Cc, pointing at a small-Cc + high-gm region. Study 2 (focused space, seed 100, 16 trials): trial 6 MEASURED gain 75.16dB, UGB 62.41MHz, PM 152.4deg — all three spec targets pass with margin.
- **Decision**: (1) Per-trial SimError is caught: empty metric set, status failed, verdict `SimError: <taxonomy-category>: <detail>`, observed at penalty cost 1e6 (above any measured miss, so TPE avoids dead zones); missing backend still raises before trial 1 (configuration error, not trial data). (2) Cost gains a PM shortfall term; `space` override added as an OPTIONAL parameter (v0.1 additive, no bump). (3) The committed live demo re-simulates the exact study-2 winning point and asserts spec compliance on MEASURED values (deterministic deck: seed-independent numerics, reproduced across runs). Advisory flag: winning Rz=7953Ω sits at the 8000Ω bound — moot given +92deg PM margin; recorded, proceeding flagged.
- **Rationale**: Followed the repo's own Stage 4 pattern (test_optimize_demo.py:149-151) for failed trials instead of inventing one. Two-stage search (wide then focused from measured data) instead of blind budget burn. The winner is an optimizer trial, not a hand tweak — the loop gets the credit, the simulator the verdict.
- **Alternatives Considered**: Hand-editing Cc on trial-1 params (rejected: story must stay optimizer-found, simulator-verified); asserting spec-pass on study-ledger values without re-simulation (rejected: the committed test must measure, not quote); widening the Rz bound and re-running (rejected: advisory-only, PM margin overwhelming).
- **Consequences**: Stage 6 benchmark genuinely met on silicon-model data. Checkpoint re-raised as PASS — human accept/reject/compare still required; no design auto-committed. Stage 7 on hold for the user's own UI/UX design.

---

### ADR-027: Phase-Margin Unwrap Fix + Signed Reporting (VERIFY-PM-001)
- **Date**: 2026-09-09
- **Status**: Accepted (implementation this commit; live-vector confirmation in Step 2)
- **Context**: Read-only audit of `extract_phase_margin` found it interpolating principal-value `cmath.phase` outputs and returning `180 - abs(phase)` with no unwrapping. Two compounding defects: (a) multi-pole lag past ±180° wraps (e.g. true −335° reads +25°) fabricating large positive PM for unstable loops — the committed 6F "PM 152.4° PASS" is therefore UNVERIFIED and predicted to re-measure as deeply negative or, in the kindest consistent story, ≈ +27° MARGINAL (still a spec FAIL either way); (b) the abs() form misreads even wrap-free cases (existing unit tests enshrined 0.0 and 60.0 where correct physics gives 180.0 and 120.0 under the repo's M = G(s) convention). The contract (`180° + phase(T)`) already demanded the true continuous phase — code violated its own contract, and a docstring-claimed "plausible window" assertion did not exist in code (phantom guardrail, removed).
- **Decision**: (1) `unwrap_phase_degrees` (numpy-style, ±360 compensation) applied before crossing interpolation; interpolation stays linear-fractional, identical to `extract_bandwidth` (deliberate deviation from the Phase-0 brief's log proposal: one metric, one crossing authority — sub-1% numeric difference at 10 pts/dec). (2) Anchor-invariant signed policy PM = 180° − (lag from DC phasor); negative returns instability data, never clamped; STABLE/MARGINAL/UNSTABLE at +45/+0 bounds via `PhaseMarginData` + `extract_phase_margin_detailed`; float API preserved. (3) Contract text updated to the lag formulation (fields asserted by tests untouched). (4) Two flipped unit assertions re-derived in comments; wrap/branch-cut/unstable/marginal adversarial suite added. (5) Step 2 re-runs the optimizer and re-baselines the demo under AGENT2 §10; planchanges.md ADR numbering (025/026 reused there) is stale versus this ledger — this ADR takes the next free number 027, PDK selection moves to 028.
- **Rationale**: Anchor-invariant lag differences are the only formulation preserving the live-verified cs_amp result (≈90°) while correcting wrapped cases; principal-value interpolation is unfixable in place. Cheapest correct layer: extractor + contract text + tests, zero deck/simulator changes.
