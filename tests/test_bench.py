"""R0 tests: AnalogBench registry integrity + B0/B1 benches.

Registry assertions run everywhere (pure metadata). B0/B1 without a backend
assert fail-closed error data (base); live benches assert measured behavior
with evidence (EDA only).
"""
...
def test_deferred_benches_raise_with_owner(tmp_path: Path) -> None:
    with pytest.raises(NotImplementedError, match="R0-2"):
        run_bench("B2", db_path=str(tmp_path / "b.sqlite"))
