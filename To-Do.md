...
- **Active Phase**: R0-5 COMPLETE — async optimizer end-to-end (supervisor + UI slice) landed; CI smoke-red fixed; full gate green at HEAD.
- **Code Status**: ✅ HEAD `7c4ca71` in sync with `origin/main`. Gate green (base 388 passed + 47 skipped, ruff clean, mypy strict clean on 125 files — re-verified 2026-09-14; one untracked scratch probe `scripts/_probe_fcn.py` was the only ruff failure, deleted).
- **Blocking**: RFC-002 Grounded Tutor draft (`docs/rfc/rfc-002-grounded-tutor.md`) awaits human review — NO tutor code until its 4 blocking questions are verdict. Non-blocking follow-up: golden byte-identical netlist asserts (substring asserts stand).
...
- [-] **Stage 7B — HTTP API service** (7B-1 `03635a8`, 7B-2a `1cc858f`, 7B-2b `5b8c80d`, 7B-3 `ca155c4`)
  ...
  - [x] R0-4d-2 diff-pair testbench (trinity complete): third fingerprint (3nfet+2pfet, eight-net mirror-load structure); single-ended 0.7–1.1 V sweep on inp with the R0-2b bias recipe; DC slope vs AC mirror-side gain cross-checked within 10% (probe: DC 8.31) plus the diode-side attenuation split as second anchor; rows persist. Base 372+46; EDA measure file 7+2. Inverter → CS → diff-pair: the fundamental analog trinity is now measurable.
  - [x] R0-4e declared-load bandwidth (first real MHz): AC decks take Stage 3 1 pF precedent via additive ac_extra_lines (DC decks byte-identical to proven recipes). MEASURED INV 56.9 MHz, CS 20.709046 MHz (reproduces Stage 3 2.07e7 to 4 sig figs through an independent path). UGBW Explorer row populates; e2e asserts MHz in the result cell (spec-column false-positive caught and fixed). Base 372+46; EDA measure file 7+2; Playwright 4/4. No TS source changes (spec only).
  - [x] RFC-001 async optimizer APPROVED with 4 binding verdicts (300 s trial timeout; default 15 / ceiling 30 trials; cancel kills sims+studies; mandatory scalarizer — booleans never reach TPE). Spec in docs/rfc/rfc-001-async-optimizer.md; ADR-035 recorded.
  - [x] R0-5 Commit 1 (§9.1) study supervisor: scalarizer (pure, gradient-ordered) + spawned supervisor proc (ask→instantiate→measure→tell→record per trial, mock/mock-slow/spec objectives, thread-timeout wrapper, per-trial heartbeat) + submit_study/cancel_job/list_trials (concrete-only) + 3 routes + boot reconciliation (Python-side timestamps, optimize-only). RFC amended to as-built (trial linkage deferred, no rollup column, validator-per-trial PDK). Base 386+47; EDA live 3-trial CS study green in 4.3 min.
  - [x] R0-5 Commit 2 (§9.3) optimizer UI slice: create_spec + POST /specs (study targets without raw SQL); api.ts study wrappers; store study state/poll/stop (demo: CS gain≥5 + UGBW≥1MHz, 3 trials, declared in UI); sim Stop now kills backend jobs too (verdict 3); Explorer trial table + best banner + convergence polyline in shared PlotPane. Playwright 5/5 (stream + cancel + complete, 11.4 min). Fixed own test-scoping bug (dialog text vs canvas guidance substring).
  - [x] CI smoke red fix: unused loop var failed ruff inside `make test` (Commit 2 went up without a full local gate — process lapse, gate now mandatory pre-push); .dockerignore excludes UI generated weight (node_modules/.output/.wrangler/test-results/dist) so `COPY . .` stops shipping ~1 GB into the image. Local ruff + mypy + full base 388+47 green. Re-run requested (59s fail profile matches build/context-stage, not tests).
- [x] Tutor RFC → FROZEN per ADR-036 (Option 1, human-directed 2026-09-14). Synthesis tutor off the table (Golden-Rule violation); v0 UI wire declined; v0-backend `26d1176` stays in tree as frozen tested code.
  - RFC-002 draft written (`docs/rfc/rfc-002-grounded-tutor.md`, 2026-09-12/14): v0 explain-or-refuse chat route + 20-prompt refusal Eval + AssistPanel rewire; v1 CandidateCircuitIR Review modal. BLOCKED on human verdicts to its §10 questions (classifier type, Eval authorship, token caps, feature flag). No code until accepted.
  - ACCEPTED 2026-09-14 (all 4 verdicts = RFC proposals: keyword rules; 20-prompt Eval with ChatGPT-Plus adversarial half ordered as follow-up; log-only metering; CI-green exposure, no flag).
...
- [-] **Active: canonical EDA pivot (ADR-036)**
  - [x] Track A1 `07ed471`: B5 folded-cascode fixture (`sim/folded_cascode.py`, template-default sizes) + B5 gate test + deferral advanced B5→B6. Evidence: ruff + mypy clean, bench file 13+5, full base 413+47 green.
  - [x] Track A2: `_run_b5` runner + dispatch + live bench tests. Bias recipe probed live on EDA (16-combo grid: NMOS-strong parks out at vss, PMOS-strong at vdd; winner tail=1.0/bp=1.0/n1=0.5/n2=1.0/vcm=0.9). MEASURED DC 28.070 == AC 28.070 @trip 0.870V, UGB 39.6kHz @1pF, rails 0.063–1.775. PM deliberately unasserted (open-loop PM ill-defined; acceptance text updated). Evidence: EDA `-k b5` 3 passed + 1 skip; full base 414+48 green.
  - [x] Track B1 `2e0af41`: `EngineV01.run_corners` (5-envelope inverter sweep, per-corner jobs, fail-soft) + POST /corners/run + `tests/test_api_corners.py` (base fail-closed + EDA live dispersion proof). First red was client ReadTimeout (5×20.5 s sweep > 60 s budget, MEASURED single-corner wall) — test-only 300 s fixture timeout per R0-4b precedent, no product change. Evidence: EDA file 4+2; full base 419+49 green.
...
analyzer traces, 1.9 min).
  ...
  - [x] B7a `6a2f265`: `sim/bandgap.py` two-PNP core + gate test + `metrics/tempco.py` + hand-golden unit tests. Full base 429+50 green.
  - [x] B7b: `_run_b7` (K=9.0 declared, ideal-R/IDC deck assumption documented) + dispatch + live tests; deferral test retired (zero deferred benches). MEASURED tempco tt 57.44 / ff 68.90 / ss 43.47 / fs,sf 57.44 ppm/°C, Vref 1.227–1.236V; band 100 clears worst ff. Readout: fs/sf==tt at same VDD (bipolar tt-only inference); tempco moves with supply. Evidence: EDA b7-live green at <100; full base 432+51 green.
  - [x] Golden netlist asserts (Error 2): NOT/NAND fragments + Miller AC deck → `tests/golden/*.cir` byte-identical; inverter micron deck → inline `==`. CONFIRMED by human 2026-09-15 (electrical correctness, node order, d/g/s/b, W/L in meters, single-ended AC drive verified; 15 passed in 249s).
- [x] Quick-wins debt batch: CORS origins tightened to the two loopback dev URLs (wildcard+credentials rejected by browsers — VERIFIED in `api/server.py:323-324`); Misc.tsx signoff/metrics/margins fiction replaced with run-state truth + live Measured block; test_digital_gates.py deck assembly via assemble_transient (VERIFIED `tests/test_digital_gates.py:24,124` — micron string-replace debt retired).
  ...
  - [ ] Follow-ups (non-blocking): ~~CORS `allow_origins *` + credentials combo in server.py is misconfigured (browsers reject) — tighten to localhost origins~~ DONE (loopback-only, verified); ~~`test_digital_gates.py` hand-rolled micron string-replaces should use `assemble_transient`~~ DONE (verified); ~~netlist asserts are substring, not golden byte-identical~~ DONE (hand-verified goldens committed 2026-09-15).
...
- [-] **Stage 8 — Physical Design** (spike recon done 2026-09-15: pya via `klayout -b -r` proven; magic 8.3 + netgen + sky130A.magicrc/setup.tcl present; layers quoted from `sky130A.lyp`)
  ...
  - [-] 🔴 HUMAN CHECKPOINT: visual layout review before PCell reuse (DRC proof + interim PNG ready).
