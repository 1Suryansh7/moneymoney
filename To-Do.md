...
- [ ] Tutor RFC (after optimizer UI green): failure-explanation-first, citation refusal, proposal gates.
  ...
  - [x] v0-backend commit `26d1176`: `ai/tutor.py` keyword classifier + `EngineV01.chat` + POST /copilot/chat + `tests/test_tutor_refusal.py` (20-prompt Eval). Evidence: ruff + mypy-strict clean, new file 24 passed, full base 412+47 green. Ledger entry in Tasks_Comp rides with the next commit.
  - [ ] v0-frontend: AssistPanel rewire + Playwright (grounded chips / refusal). v1: CandidateCircuitIR path + Review modal + `human_decision` proof. Docs sync: ADR-036/037 + push. Follow-up: ChatGPT-Plus adversarial Eval hardening.
